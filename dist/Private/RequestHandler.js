...
class RequestHandler {
    ...
    setBaseURL(url = 'https://api.hypixel.net/v2') {
        if (!url.startsWith('http') || !url.includes('://'))
            throw new Error(Errors.INVALID_BASE_URL);
        if (url.endsWith('/'))
            throw new Error(Errors.INVALID_BASE_URL_SLASH);
        this.BASE_URL = url;
        return this;
    }
    async request(endpoint, options) {
        ...
        if (res.status >= 500 && res.status < 528) {
            throw new Error(Errors.ERROR_STATUSTEXT.replace(/{statustext}/, `Server Error : ${res.status} ${res.statusText}`));
        }
        const parsedRes = (await res.json());
        if (res.status === 400) {
            throw new Error(Errors.ERROR_CODE_CAUSE.replace(/{code}/, '400 Bad Request').replace(/{cause}/, parsedRes.cause || 'UNKNOWN'));
        }
        if (res.status === 403)
        ...
        if (res.status === 429)
            throw new Error(Errors.RATE_LIMIT_EXCEEDED);
        if (res.status !== 200) {
    ...
    async toUUID(input) {
        ...
        if (typeof input !== 'string')
            throw new Error(Errors.UUID_NICKNAME_MUST_BE_A_STRING);
        if (this.client.functions.isUUID(input))
        ...
        const parsedRes = (await res.json());
        if (res.status === 400) {
            throw new Error(Errors.ERROR_CODE_CAUSE.replace(/{code}/, '400 Bad Request').replace(/{cause}/, parsedRes.cause || ''));
        }
        if (res.status !== 200) {
            throw new Error(Errors.ERROR_STATUSTEXT.replace(/{statustext}/, res.statusText));
        }
