import { CACTUS, CARROT, COCOA_BEANS, CustomPetLeveling, DEFAULT_LEVELING_XP, DEFAULT_SKILL_CAPS, DUNGEONEERING_XP, GARDEN_XP, HOTF_XP, HOTM_XP, INFINITE, MELON, MOONFLOWER, MUSHROOM, NETHER_WART, POTATO, PUMPKIN, PetLevels, RUNECRAFTING_XP, SLAYER_XP, SOCIAL_XP, SUGAR_CANE, SUNFLOWER, WHEAT, WILD_ROSE, petRarityOffset } from "./constants.js";
import { parse, simplify } from 'prismarine-nbt';
export async function decode(base64, isBuffer = false) {
...
export function getPetLevel(petExp, type, rarity) {
    const rarityOffset = type === 'BINGO' ? (CustomPetLeveling[type]?.rarityOffset?.[rarity] ?? petRarityOffset[rarity]) : 0;
    const maxLevel = type === 'GOLDEN_DRAGON' ? CustomPetLeveling[type]?.maxLevel : 100;
    const levels = PetLevels.slice(rarityOffset, rarityOffset + maxLevel - 1).concat(type === 'GOLDEN_DRAGON' ? CustomPetLeveling[type]?.petLevels : PetLevels);
    let level = 1;
    let xpMaxLevel = 0;
    let currentXp = 0;
...
export function getSlayerLevel(slayer, xp) {
...
export function getXpTable(type) {
    const SKILL_TABLES = {
...
export function getLevelByXp(xp, extra = { type: 'default' }) {
...
export function SkyBlockProfileHasMe(profile) {
    return profile.me !== null;
}
//# sourceMappingURL=skyBlockUtils.js.map