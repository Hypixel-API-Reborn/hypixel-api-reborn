...
export const DuelsDivisionsRequirements = {
...
export function getDuelsTitle(score, isAllModes = false) {
    ...
    const titleData = divisions.find(({ req }, index) => score >= req && score < (divisions[index + 1]?.req ?? Infinity)) ?? divisions[0];
    if (!titleData)
        throw new HypixelAPIRebornError(Errors.INVALID_DUELS_TITLE_REQUIREMENT);
    const { req, step, title, max } = titleData;
    const division = Math.min(max, step ? Math.floor((score - req) / step) + 1 : 1);
    return `${title}${division > 1 ? ` ${Romanize(division)}` : ''}`;
}
